# Prerequisites
  ...
  1. Install MongoDB Community Server 8 with default user or use MongoDB Atlas
  2. Install NodeJS 22 with NPM

# Quick Start

  1. Configure the server port and MongoDB variables in `.env` and `/src/frontend/src/app/environment.ts` files if necessary.
  ...
  3. Create a Firebase project with Authentication enabled (Google as OAuth provider), then:
      ...
      2. Create `src/frontend/.env.local` file and save your firebase config object (e.g. web credentials) as a stringified JSON as `NG_APP_FIREBASE_CONFIG`
